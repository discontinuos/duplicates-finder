...
namespace finder
{
	class FindGroups
	{
		...
		public FindGroups(int sizeLimit)
		{
		...
		public void Process(string outpath)
		{
			...
			using (SQLiteCommand cmd = new SQLiteCommand(stm, conn))
			{
				using (SQLiteDataReader rdr = cmd.ExecuteReader())
				{
					...
					{
						n++;
						if (n % 100 == 0) Status.Update("Pares: " + (Math.Floor(((double) n / total) * 10000))/100 + " % (" + n + " de " + total + "). \n\nTranscurridos: " + ((int) (DateTime.Now - t).TotalMinutes) + " minutos. Encontrados: " + found);
			...
			int gruposEncontrados = Status.GetTableSize(conn, "Groups_" + SIZE_LIMIT);
			if (gruposEncontrados  > 0)
				Status.Hide("Se encontraron exitosamente " + gruposEncontrados.ToString() + " grupos.");
			else
				Status.Hide("No se encontraron grupos con el punto de corte (tamaño mínimo) establecido.");
			conn.Dispose();
